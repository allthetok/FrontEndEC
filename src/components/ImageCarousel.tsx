...
const ImageCarousel = ({ colors, colorQuery }: ImageProps ) => {
	const colorsFiltered: Colors[] = colors.filter((indColor: Colors) => indColor.color === colorQuery)
	return (
		<div className='lg:w-1/2 w-full lg:h-auto h-64'>
			<CarouselProvider
				naturalSlideWidth={496}
				naturalSlideHeight={496}
				totalSlides={colorsFiltered[0].images.length}
				infinite>
				<Slider>
					{colorsFiltered[0].images.map((image: string, index: number) => (
